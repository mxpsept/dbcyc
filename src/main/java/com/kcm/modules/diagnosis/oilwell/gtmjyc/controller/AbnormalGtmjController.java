package com.kcm.modules.diagnosis.oilwell.gtmjyc.controller;
 ...
 */
@RestController
@RequestMapping("/abnormalGtmjEntity")
@Api(tags = "功图面积异常接口")
@RequiredArgsConstructor
public class AbnormalGtmjController {

    private final AbnormalGtmjService abnormalGtmjService;
    ...
    public AjaxResult findListByPage(@RequestParam(required = false, defaultValue = "1") Integer page,
        ...
        try {
            return success(ResultCode.SUCCESS_QUERY, abnormalGtmjService.findAllByPage(page, size));
        } catch (Exception e) {
    ...
    public AjaxResult findListsByPage(@RequestBody DymParamDto dto,
        ...
        try {
            return success(ResultCode.SUCCESS_QUERY, abnormalGtmjService.findAllByPageAndCon(dto,page,size));
        } catch (Exception e) {
            return error(ResultCode.ERR_SQL_SELECT_ERROR, e.getMessage());
        }
    }
